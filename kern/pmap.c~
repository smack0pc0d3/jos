...
mem_init_mp(void)
{
    ...
    for (i = 0; i < NCPU; i++) {
        ...
        boot_map_region(kern_pgdir, kstacktop_i-KSTKSIZE, ROUNDUP((KSTKSIZE),
                    PGSIZE), PADDR((void *)percpu_kstacks[i]), (PTE_P | PTE_W));
        /*
        boot_map_region(kern_pgdir, kstacktop_i - (KSTKSIZE + KSTKGAP), ROUNDUP((KSTKGAP),
                    PGSIZE), 0x00, PTE_P);
        */
    }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if (!create && !((pde_t)*pde & PTE_P))
        return NULL;
    else if ((pde_t)*pde & PTE_P)
        pgtbl = pa2page((physaddr_t)*pde);
    else {
        if (!(pgtbl = page_alloc(ALLOC_ZERO)))
            return NULL;
        pgtbl->pp_ref++;
        *pde = page2pa(pgtbl) | PTE_P | PTE_W | PTE_U;
    }
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    if (!(pg = page_lookup(pgdir, va, &pte)))
        return;
    page_decref(pg);
    *pte = 0;
...
mmio_map_region(physaddr_t pa, size_t size)
{
    ...
    if (base+ROUNDUP(size, PGSIZE) > MMIOLIM)
        panic("MMIOLIM has been exceeded");
...
check_page_installed_pgdir(void)
{
    ...
    tlb_invalidate(kern_pgdir, (void *)PGSIZE);
